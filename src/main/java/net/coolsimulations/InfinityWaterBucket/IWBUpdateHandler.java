...
public class IWBUpdateHandler {
	...
	public static void init() {
		...
		try {
            URL url = new URL("https://coolsimulations.net/mcmods/infinity-water-bucket/versionchecker119.txt");
		...
		try {
			URL url = new URL("https://coolsimulations.net/mcmods/infinity-water-bucket/updateinfo119.txt");
			Scanner s = new Scanner(url.openStream());
		...
		if(latestVersion != null) {
			...
			if(latestVersion.equals("ended")) {
				...
				updateInfo = Component.translatable("iwb.update.display3", new Object[] {iwb, MCVersion});
			...
			if(!latestVersion.equals(IWBReference.VERSION) && !latestVersion.equals("ended")) {
				...
				updateInfo = Component.translatable("iwb.update.display1", new Object[] {iwb, version});
				updateInfo.withStyle(ChatFormatting.YELLOW);
